    ...
    if(isset($_POST['entrada'])) {
        ...
        if ($resultado = mysqli_query($conn, $query)) {
            while ($fila = mysqli_fetch_row($resultado)) {
                ...
                echo $rolUsuario;
                echo "<br>";
                $nombreCompleto = $apellidoUno . " " . $apellidoDos . ", " . $nombre . " - " . $nombreCentro;
                echo $nombreCompleto;
                if ($rolUsuario == "U") {
                    echo "<br>";
                    echo "El Usuario logueado es tipo (U)suario";
                    header("location: principal.php?apu=$apellidoUno & apd=$apellidoDos & nom=$nombre & cen=$nombreCentro & num=$centro & ide=$usuariorecibido");
                }
                if ($rolUsuario == "T") {
                    echo "<br>";
                    echo "El Usuario logueado es tipo (T)écnico";
                    header("location: principalUno.php?apu=$apellidoUno & apd=$apellidoDos & nom=$nombre & cen=$nombreCentro & num=$centro & ide=$usuariorecibido");
                }
                if ($rolUsuario == "A") {
                    echo "<br>";
                    echo "El Usuario logueado es tipo (A)uxiliar";
                    header("location: principal3.php?apu=$apellidoUno & apd=$apellidoDos & nom=$nombre & cen=$nombreCentro & num=$centro & ide=$usuariorecibido");
                }
                if ($rolUsuario == "P") {
                    echo "<br>";
                ...
                if ($rolUsuario == "Z") {
                    ...
                    header("location: principalsuper.php?apu=$apellidoUno & apd=$apellidoDos & nom=$nombre & cen=$nombreCentro & num=$centro & ide=$usuariorecibido & rol=$rolUsuario");
                }
                if ($rolUsuario == "Y") {
                ...
                if ($rolUsuario == "X") {
                    header("location: emergente.php?apu=$apellidoUno & apd=$apellidoDos & nom=$nombre & cen=$nombreCentro & num=$centro & ide=$usuariorecibido & rol=$rolUsuario");
                }
                if($rolUsuario == "U" && (($usuario == "12345678z" || $usuario == "12345678Z")) || ($usuario == "11111111H" || $usuario == "11111111h")){
                    header("location: emergente3.php");
                }
