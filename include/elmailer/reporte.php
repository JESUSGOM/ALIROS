<?php
    ...
    $fila = 35;
    $barra = " / ";
    $sfpdf = new FPDF('P','mm','A4');
    $textotitulo = "Informe mensual Servicio ITC por Laborsord SL (CEE) para el periodo: ".$esmes."/".$esaño.". Prestado en ".$deno. "";
    $textotitulo .= ".pdf";
    ...
    $sfpdf->SetFillColor(0,166,93);
    $sfpdf->SetTextColor(255,255,255);
    $sfpdf->Cell(75, 5, utf8_decode('Cliente:'),1,0,'L',true);
    $sfpdf->Cell(75, 5, utf8_decode('Fecha:'),1,0,'L',true);
    $fila = $fila + 6;//Item1
    if($fila >= 279){
        $fila = 35;
    }
    ...
    if($cen == 1){
        $sfpdf->Cell(75,5, utf8_decode('ITC SC - 0796/2021'),0, 0,'L');
    }
    if($cen == 2){
        $sfpdf->Cell(75,5, utf8_decode('ITC SC - 0107/2021'),0, 0,'L');
    }
    ...
    $sfpdf->SetFillColor(0,166,93);
    $sfpdf->SetTextColor(255,255,255);
    $sfpdf->Cell(75, 5, utf8_decode('Dirección:'),1,0,'L',true);
    $sfpdf->Cell(75, 5, utf8_decode('Persona de contacto:'),1,0,'L',true);
    $fila = $fila + 6;//Item3
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->SetFillColor(0,166,93);
    $sfpdf->SetTextColor(255,255,255);
    $sfpdf->Cell(75, 5, utf8_decode('CIF.:'),0,0,'L',true);
    $fila = $fila + 6;//Item6
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->SetTextColor(255,255,255);
    $sfpdf->Cell(150, 5, utf8_decode('Licitación.'),0,0,'L',true);
    $fila = $fila + 6;//Item8
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->SetFillColor(255,255,255);
    $sfpdf->SetTextColor(0,0,0);
    $sfpdf->Cell(150,5,utf8_decode('Servicio de auxiliares de recepción para varios centros del Instituto Tecnológico'),0,0,'L');
    $fila = $fila + 5;//Item9
    if($fila >= 279){
        $fila = 35;
    }
    ...
    echo $esmes;
    */
    $sfpdf->Cell(150,5,utf8_decode('Informe mensual de '.$Mesmes.''),0,0,'L');
    $fila = $fila + 7;//Item11
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->SetXY(15,$fila);//Numero fila 111
    $sfpdf->Write(5,utf8_decode('Se trata del edificio que alberga las oficinas principales del ITC en la provincia de Santa'));
    $fila = $fila + 5;//Item13
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 116
    $sfpdf->Write(5,utf8_decode('Cruz de Tenerife. Dispone de dos plantas con una superficie a proximada de unos 4.699  '));
    $fila = $fila + 5;//Item14
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 121
    $sfpdf->Write(5,utf8_decode('m2, que albergan dependencias destinadas a oficinas administrativas, despachos  de la '));
    $fila = $fila + 5;//Item15
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//numero fila 126
    $sfpdf->Write(5,utf8_decode('Dirección, almacenes, archivos, CPD, laboratorios, nido  de empresas, y  determinadas '));
    $fila = $fila + 5;//Item15
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 131
    $sfpdf->Write(5,utf8_decode('zonas comunes como Salón de actos, salas de reuniones, aulas de formación, cafetería.'));
    $fila = $fila + 5;//Item16
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 136
    $sfpdf->Write(5,utf8_decode('Los aspectos más relevantes en relación al desarrollo de dos bloques de tareas que '));
    $fila = $fila + 5;//Item17
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 141
    $sfpdf->Write(5,utf8_decode('componen el servicio son los siguientes:'));
    $fila = $fila + 5;//Item18
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->Write(5,utf8_decode('Labores de portería:'));
    $sfpdf->SetFont('Arial','',11);
    $fila = $fila + 5;//Item19
    if($fila >= 279){
        $fila = 35;
    ...
    $sfpdf->Write(5,utf8_decode('El edificio dispone de un acceso principal situado en la 1ª Fase, un acceso auziliar en la '));
    $fila = $fila + 5;//Item20
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 158
    $sfpdf->Write(5,utf8_decode('2ª Fase y un acceso de servicio para tareas de carga/descarga.'));
    $fila = $fila + 5;//Item21
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 163
    $sfpdf->Write(5,utf8_decode('El recinto que alberga el edificio se encuenta vallado y dispone de una puerta con motor'));
    $fila = $fila + 5;//Item22
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 168
    $sfpdf->Write(5,utf8_decode('para el acceso de vehículos y una puerta de paso para peatones. El centro también tiene'));
    $fila = $fila + 5;//Item23
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 173
    $sfpdf->Write(5,utf8_decode('de espacios exteriores alrededor del cuerpo frontal de la edificación, con una via interior,'));
    $fila = $fila + 5;//Item24
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 178
    $sfpdf->Write(5,utf8_decode('espacios ajardinados y aparcamiento al aire libre.'));
    $fila = $fila + 5;//Item25
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 183
    $sfpdf->Write(5,utf8_decode('La recepción del edificio y cuarto de servicio con el cuadro de control de llaves y almacén'));
    $fila = $fila + 5;//Item26
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 188
    $sfpdf->Write(5,utf8_decode('de material de oficina se ubican junto al acceso de la 1ª Fase.'));
    $fila = $fila + 6;//Item27
    if($fila >= 279){
        $fila = 35;
    }
    ...
    $sfpdf->Write(5,utf8_decode('El edificio se cataloga como "edificio de pública concurrencia" en virtud de la naturaleza'));
    $fila = $fila + 5;//Item29
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 204
    $sfpdf->Write(5,utf8_decode('de actividades que desarrolla el ITC.'));
    $fila = $fila + 5;//Item30
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 209
    $sfpdf->Write(5,utf8_decode('El edificio contiene dependencias de la sede insular del ITC, como oficinas en régimen'));
    $fila = $fila + 5;//Item31
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 214
    $sfpdf->Write(5,utf8_decode('de alquiler con capacidad para una veintena de empresas alojadas.'));
    $fila = $fila + 5;//Item32
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 219
    $sfpdf->Write(5,utf8_decode('En la recepción se ubica la centralita telefónica del centro, equipos generales básicos del'));
    $fila = $fila + 5;//Item33
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 224
    $sfpdf->Write(5,utf8_decode('centro tales como la centralita contra incendio. Además se hace el control de paquetería'));
    $fila = $fila + 5;//Item34
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 229
    $sfpdf->Write(5,utf8_decode('correspondencia control de acceso de personal y/o visitantes y recepción de llamadas de '));
    $fila = $fila + 5;//Item35
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 234
    $sfpdf->Write(5,utf8_decode('teléfono recibidas en la centralita. También se realiza la labor de monitorización de los demás'));
    $fila = $fila + 5;//Item36
    if($fila >= 279){
        $fila = 35;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 239
    $sfpdf->Write(5,utf8_decode('puntos de acceso existentes en el edificio.'));
    $fila = $fila + 5;//Item37
    if($fila >= 279){
        $fila = 35;
    }
