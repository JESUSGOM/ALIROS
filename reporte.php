<?php
    ...
    $textotitulo = "Informe mensual Servicio ITC por Envera Empleo, S.L.U. (CEE) para el periodo: ".$esmes."/"
        .$esnano.". Prestado en ".$deno. "";
    $textotitulo .= ".pdf";
    ...
    //$sfpdf->Image('img/Imagen1.jpg', 5, 20, 260, 170, 'JPG');
    ...
    $sfpdf->SetFillColor(255,255,255);
    $sfpdf->SetTextColor(0,0,0);
    $sfpdf->Cell(260,5,utf8_decode('Servicio de auxiliares de recepción para varios centros del Instituto 
    Tecnológico de Canarias, S.A. (ITC)'),0,0,'L');
    $fila = $fila + 5;//Item9
    if($fila >= 203){
        $fila = 20;
    }
    ...
    */
    ...
    'Se trata del edificio que alberga las oficinas principales del ITC en la provincia de Santa Cruz de Tenerife. 
    Dispone de dos plantas con una superficie aproximada de unos'));
    $fila = $fila + 5;//Item13
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode(
    '4.699 m2,  que albergan dependencias destinadas a oficinas administrativas,  despachos de la Dirección,  
    almacenes,  archivos,  CPD,  laboratorios,  nido para empresas,'));
    $fila = $fila + 5;//Item14
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode(
    'El edificio dispone de un acceso principal situado en la 1ª Fase, un acceso auziliar en la 2ª fase y un 
    acceso de servicio para tareas de carga/descarga.'));
    $fila = $fila + 5;//Item20
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode('El recinto que alberga el edificio se encuenta vallado y dispone de una puerta 
    con motor para el acceso de vehículos y una puerta de paso para peatones.'));
    $fila = $fila + 5;//Item22
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $fila = $fila + 5;
    $sfpdf->Write(5,utf8_decode('El centro también tiene de espacio exteriores alrededor del cuerpo frontal de la 
    edificación, con una via interior, espacios ajardinados y aparcamientos al aire libre'));
    //$fila = $fila + 5;//Item23
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode('La recepción del edificio y cuarto de servicio con el cuadro de control de llaves
     y almacén de material de oficina se ubican junto al acceso de la 1ª Fase.'));
    $fila = $fila + 10;//Item26
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->SetFont('Arial','B',8);
    $sfpdf->Write(5,utf8_decode('El edificio se cataloga como "edificio de pública concurrencia" en virtud de la 
    naturaleza de actividades que desarrolla el ITC.'));
    $fila = $fila + 5;//Item29
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->SetFont('Arial','B',8);
    $sfpdf->Write(5,utf8_decode('El edificio contiene dependencias de la sede insular del ITC, como oficinas en 
    régimen de alquiler con capacidad para una veintena de empresas alojadas.'));
    $fila = $fila + 5;//Item31
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode('En la recepción se ubica la centralita telefónica del centro, equipos generales 
    básicos del centro tales como la centralita contra incendio.'));
    $fila = $fila + 5;//Item33
    if($fila >= 203){
        $fila = 20;
    }
    $sfpdf->SetXY(15,$fila);//Numero fila 224
    $sfpdf->Write(5,utf8_decode('Además se hace el control de paquetería, correpondencia, control de acceso de 
    personal y/o visitantes, así ccomo la recepción de llamadas de teléfono'));
    $fila = $fila + 5;//Item34
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->Write(5,utf8_decode('recibidas en la centralita. También se realiza la labor de monitorización de los 
    demás puntos de acceso existentes en el edificio.'));
    $fila = $fila + 5;//Item36
    if($fila >= 203){
        $fila = 20;
    }
    ...
    $sfpdf->SetXY(15, $fila);
    $sfpdf->SEtFont('Arial', 'B', 6);
    $sfpdf->Write(5, utf8_decode('C/Bahía de Pollensa nº 25. 28042-Madrid   CIF.:B-86363603. Tlf.: 91 746 20 81 
    Fax.: 91 747 71 45'));
    $fila = $fila + 3;
    $sfpdf->SetXY(15, $fila);
    $sfpdf->SEtFont('Arial', 'B', 6);
    $sfpdf->Write(5, utf8_decode('Inscrita en el Registro mercantil TOMO: 29.509. LIBRO: 0. FOLIO: 111. SECCIÓN 8. 
    HOJA: M-531080'));
    $fila = $fila + 3;
    $sfpdf->SetXY(15, $fila);
    $sfpdf->SEtFont('Arial', 'B', 6);
    $sfpdf->Write(5,utf8_decode('CEE DE TENERIFE - C/Cruz Caridad nº 42. 38350-Tacoronte. Santa Cruz de Tenerife. 
    Tlf.:922 56 00 37'));
    $sfpdf->SetDrawColor(155, 155, 155);
    ...
    $sfpdf->SetFont('Arial','',11);
    $sfpdf->Write(5,utf8_decode('Las tareas a desarrollar por los auxiliares de Recepción son, de carácter 
    enunciativo y no'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    $sfpdf->Write(5,utf8_decode('limitativo, y con ciertas variaciones en función de las características de cada 
    centro,  son'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    $sfpdf->Write(5,utf8_decode('b) Atención de la centralita telefónica, gestión de fax central y registro de los
     visitantes.'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    $sfpdf->Write(5,utf8_decode('c) Atención, guía y orientación a visitas, proveedores, clientes de ITC., 
    empresas alojadas.'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    $sfpdf->Write(5,utf8_decode('d) Custodia del cuadro de llaves de servicio y control de entregas y devolución 
    de las mismas'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    $sfpdf->Write(5,utf8_decode('i) Participar en la feceta de comunicación de incidencias y/o alarmas en el 
    edificio.'));
    $fila = $fila + 5;//Item
    if($fila >= 203){
        $fila = 20;
    ...
    while($esfila = mysqli_fetch_row($rt)){
        $fila = $fila + 4;
        $sfpdf->SetXY(15,$fila);
        //$columna1 = strval(utf8_decode("'.substr($esfila[2],6,2).'"/"'.substr($esfila[2],4,2).'"/"'
        //.substr($esfila[2],0,4).'"));
        //$columna2 = strval(utf8_decode("'.$esfila[4].'"));
        //echo utf8_decode("'.substr($esfila[2],6,2).'"/"'.substr($esfila[2],4,2).'"/"'.substr($esfila[2],0,4).'");
        $sfpdf->Cell(25,5,utf8_decode(substr($esfila[2],6,2)."/".substr($esfila[2],4,2).
            "/".substr($esfila[2],0,4)),0,0,'L',false);
        //$sfpdf->Write(5,utf8_decode("'.substr($esfila[2],6,2).'"/"'.substr($esfila[2],4,2).'"/"'
        //.substr($esfila[2],0,4).'"));
        $fila = $fila + 4;
        $sfpdf->SetXY(15,$fila);
    ...
    //}
    $sfpdf->SetXY(15, $fila);
    $sfpdf->Cell(40,5,utf8_decode('Total visitas recibidas en el mes de ' .$esmes. ' es de '.$totalvisitas.''),
        0,0,'L',false);
    $fila = $fila+5;
    $sfpdf->SetXY(15,$fila);
    $sfpdf->SetFont('Arial','B',11);
    ...
    $fila = $fila+5;
    $qq = "SELECT DISTINCT MovDestino FROM Movadoj WHERE MovCentro = '" .$cen. "' AND MovFechaEntrada LIKE '"
        .$dmesano."' ";
